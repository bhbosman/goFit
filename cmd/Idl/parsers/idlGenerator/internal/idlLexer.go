...
func (y *yylexer) Error(e string) {
	...
	s := fmt.Sprintf("error: %v, fileName: %v (%v,%v)",
		e,
		currentStream.fileName,
		currentStream.lineNumber,
		currentStream.rowNumber-len(y.buf),
	)
	panic(errors.New(s))
}

func (y *yylexer) onCreateInteger(val *YYSymType) (int, error) {
	intValue, _ := __strconv__.ParseInt(string(y.buf[:len(y.buf)]), 10, 64)
	astValue := &ast.BasicLit{Value: __strconv__.Itoa(int(intValue)), Kind: token.INT}
...
func (y *yylexer) onCreateFloat(val *YYSymType) (int, error) {
	v, _ := __strconv__.ParseFloat(string(y.buf), 64)
	astValue := &ast.BasicLit{Value: __strconv__.FormatFloat(v, 'g', -1, 64), Kind: token.FLOAT}
	val.Node = UnionData[ast.Expr]{
		y.FileName(),
		y.LineNumber(),
		y.RowNumber(),
...
func (y *yylexer) OnHexValue(val *YYSymType) (int, error) {
	intValue, _ := __strconv__.ParseInt(string(y.buf[2:len(y.buf)]), 16, 64)
	val.Node = UnionData[ast.Expr]{
		y.FileName(),
		y.LineNumber(),
		y.RowNumber(),
		&ast.BasicLit{
			ValuePos: 0,
			Kind:     token.INT,
			Value:    __strconv__.Itoa(int(intValue)),
		},
	}
	return Integer_literal, nil
}
